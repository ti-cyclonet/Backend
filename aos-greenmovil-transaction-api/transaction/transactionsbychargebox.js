...
exports.handler = async (event) => {    
  ...
  await connection.query('SET time_zone = "-05:00";');
    try {
        ...
        const { chargebox_id } = event.pathParameters;
       ...
       if (event.queryStringParameters &&
        ...
        const query = ` SELECT tr.*, co.connector_id, fu.fuun_name name,
                        ...
                        left join functional_units fu on fugr_fuid=fu.fuun_id
                            WHERE tr.connector_pk in (
                            SELECT connector_pk 
                            FROM connector 
                            WHERE charge_box_id = ? AND start_event_timestamp BETWEEN STR_TO_DATE(?, '%Y-%m-%dT%H:%i:%s') AND STR_TO_DATE(?, '%Y-%m-%dT%H:%i:%s')
                        ) 
                        order by start_event_timestamp desc;`;

        const queryParams = [chargebox_id,startdate,enddate];
